...
export function CreateTeamModal({ onClose, onTeamCreated }: { 
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      try {
        const res = await fetch(`http://${process.env.NEXT_PUBLIC_BACKEND_URL}/api/contacts`, {
          credentials: "include",
        });
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`http://${process.env.NEXT_PUBLIC_BACKEND_URL}/api/teams/create`, {
        method: "POST",
        headers: {
